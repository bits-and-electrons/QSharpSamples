namespace SubtractorCircuit {
    ...
    operation SubtractorCircuit (a_Int: Int, b_Int: Int) : Int {
	    mutable result = -1;

        use b_Qubit = Qubit[BitSizeI(b_Int)] {
			ToQubitArrayI(b_Int, b_Qubit);

			Subtractor(b_Qubit, a_Int);
    ...
    operation Subtractor (b_Qubit: Qubit[], a_Int: Int) : Unit {
		body(...) {
			use a_Qubit = Qubit[BitSizeI(a_Int)] {
				ToQubitArrayI(a_Int, a_Qubit);

				QFT_(b_Qubit);
				Adjoint ADD(a_Qubit, b_Qubit);
